...
class Model2Partitioner : public Partitioner {
private:
    // TODO 因为NE算法在设定负载均衡的时候，是直接给个参数，会导致最后一个分区可能出现顶点数较少的情况，导致负载均衡做的不够好
    // TODO 因为NE算法是以边为划分的基本单位，所以顶点数较少的情况是合理的
    // TODO 如何在NE算法的基础上去减低RF
...
public:
    Model2Partitioner(std::string input, std::string algorithm, int num_partition);
