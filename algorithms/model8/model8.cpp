...
Model8Partitioner::Model8Partitioner(BaseGraph& baseGraph,
                                     const string &input,
...
//最后一个子图就是剩下边组合而成
void Model8Partitioner::assign_remaining() {
...
void Model8Partitioner::assign_master() {
...
//    adj_directed.build_directed(edges, reverse_indices);
    ...
    for (int i = 0; i < num_vertices; i++) {
        first_vertex_set.insert(i);
    }

    LOG(INFO) << "Start Model8 partitioning...";
    ...
    if (cores > 1) {
        ...
        for (int i = 0; i < cores; ++i) {
            threads[i] = thread(&Model8Partitioner::sub_split, this, i);
        }
        total_time.start();
        Timer t;
        t.start();
        ...
        t.stop();
        // LOG(INFO) << "Model8 multi-partitioning time: " << t.get_time() << endl;
        // LOG(INFO) << "Model8 multi-partitioning finished!" << endl;
    } else {
        total_time.start();
...
void Model8Partitioner::sub_split(size_t index) {
        ...
        while(iterator != vertex_adjacent_edges[vid].end()){
            ...
            if (edge.valid()) {
                ...
                if (acquire_vertex(neighbor)) {
                    if (edge.valid()) {
                        // TODO 并发问题
                        for (auto &pair: vertex_adjacent_edges[neighbor]) {
                            auto neighbor_edge = *pair.second;
                            vid_t neighbor_neighbor = edge.first == neighbor ? edge.second : edge.first;
                            first_vertex_set.erase(neighbor_neighbor);
                            second_vertex_set.insert(neighbor_neighbor);
                        }
...
bool Model8Partitioner::sub_get_free_vertex(vid_t &vid, vid_t index) {
    LOG(INFO) << "Start sub_get_free_vertex " << index << endl;
    // 这个set是一个相对的的set，不需要考虑并发的情况，真正还是以acquire_vertex()和release_vertex()来控制
    std::lock_guard<std::mutex> lock(vertex_mutex);
    while (!first_vertex_set.empty()) {
        int size = first_vertex_set.size();
        // 使用高质量随机数生成器
        std::random_device rd;
        std::mt19937 g(rd());
        // 创建一个从0到setSize-1的均匀分布
        std::uniform_int_distribution<> distr(0, size - 1);
        // 随机选择一个索引
        int i = distr(g);
        // 使用迭代器遍历set，并跳过相应数量的元素以模拟随机选择
        auto it = first_vertex_set.begin();
        std::advance(it, i);
        // 随机选择一个索引
        distr(g);
        vid = *it;
        // 把邻居加入到二级集合
        first_vertex_set.erase(vid);
        for (auto &pair: vertex_adjacent_edges[vid]) {
            auto edge = *pair.second;
            vid_t neighbor = edge.first == vid ? edge.second : edge.first;
            first_vertex_set.erase(neighbor);
            second_vertex_set.insert(neighbor);
        }
        if (acquire_vertex(vid)) {
            return true;
        }
    }
    while (!second_vertex_set.empty()) {
        int size = second_vertex_set.size();
        std::random_device rd;
        std::mt19937 g(rd());
        std::uniform_int_distribution<> distr(0, size - 1);
        int i = distr(g);
        auto it = second_vertex_set.begin();
        std::advance(it, i);
        vid = *it;
        second_vertex_set.erase(vid);
        LOG(INFO) << vid << " from second set";
        if (acquire_vertex(vid)) {
            return true;
        }
    }
...
bool Model8Partitioner::get_free_vertex(vid_t &vid) {
    //随机选择一个节点
    vid = dis(gen);
...
void Model8Partitioner::sub_add_boundary(vid_t vid, size_t index) {
...
void Model8Partitioner::add_boundary(vid_t vid) {
    // 获取到当前分区的核心集和边界集
...
void Model8Partitioner::assign_edge(size_t index, vid_t from, vid_t to) {
...
size_t Model8Partitioner::check_edge(const edge_t *e) {
...
void Model8Partitioner::build_vertex_adjacent_edges() {
    // 遍历edges
...
//void Model8Partitioner::calculate_replication_factor() {
