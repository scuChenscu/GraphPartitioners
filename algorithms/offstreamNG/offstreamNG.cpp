...
void OffstreamNGPartitioner::assign_remaining() {
    ...
    repv(u, num_vertices) for (auto &i: adj_out[u])
            if (edges[i.v].valid()) {
                ...
                vp_set[u].set_bit_unsync(num_partitions - 1);
                vp_set[edges[i.v].second].set_bit_unsync(num_partitions - 1);
...
size_t OffstreamNGPartitioner::leastLoad(const dense_bitset& bitmap) {
