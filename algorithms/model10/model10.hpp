...
class Model10Partitioner : public EdgePartitioner {
private:
    size_t cores;
    double capacity_ratio;
    vector<double> capacity_ratios;
...
//    vector<size_t> reverse_indices;
    ...
    // 选取顶点的位置
    vector<size_t> marks;
    vector<size_t> first_vertices;
    // 二级顶点
    vector<size_t> second_vertices;
    vector<size_t> second_marks;
//    uint104_t min_load = UINT104_MAX;
//    uint104_t max_load = 0;
...
public:
    Model10Partitioner(BaseGraph& baseGraph, const string& input, const string& algorithm,
