...
DnePartitioner::DnePartitioner(BaseGraph& baseGraph,
                                     const string &input,
...
//    degree_file.close();
    ...
    edge_lock.resize(num_edges, 0);
    degree_updated.assign(num_vertices, vector<size_t>(num_partitions, 0));
}

//最后一个子图就是剩下边组合而成
void DnePartitioner::assign_remaining() {
...
void DnePartitioner::assign_master() {
...
//    adj_directed.build_directed(edges, reverse_indices);
    ...
    LOG(INFO) << "Start Dne partitioning...";
    // 把参数写入文件，NE是边分割算法，计算复制因子
    string current_time = getCurrentTime();
    stringstream ss;
    ss << "Dne"
       << endl
       << "并行分区，分区数跟核心相同，对于选进核心集的顶点，一阶邻边分配使用的是CAS操作，二阶邻边分配使用的是最小负载的原则，"
          "每次分配完要线程进行状态同步来更新顶点的度数（怎么实现这个同步？）"
    ...
    if (cores > 1) {
        ...
        for (int i = 0; i < cores; ++i) {
            threads[i] = thread(&DnePartitioner::sub_split, this, i);
        }
        ...
        t.stop();
        // LOG(INFO) << "Dne multi-partitioning time: " << t.get_time() << endl;
        // LOG(INFO) << "Dne multi-partitioning finished!" << endl;
    } else {
        total_time.start();
...
void DnePartitioner::sub_split(size_t index) {
    // LOG(INFO) << "Start sub_split " << index << endl;
    Timer t;
    t.start();
    // LOG(INFO) << capacity * capacity_ratio;
    while (occupied[index] <= capacity * capacity_ratio) {
        // LOG(INFO) << "Core " << index << " " << occupied[index];
         size_t res = accumulate(degrees.begin(), degrees.end(), 0);
         // LOG(INFO) << res;
        // 顶点可以复用
        vid_t degree, vid;
        if (min_heaps[index].get_min(degree, vid)) {
            // 从堆中获取
            min_heaps[index].remove(vid);
        } else {
            // 从顶点集合中获取顶点
            // TODO 会不会是后期顶点竞争激烈
            if(!get_free_vertex(vid, index)) {
                break;
            }
        }
        set<size_t> vertices_need_updated;
        ...
        // 这里只需要把顶点加入边界集合，和分配一阶边（更新顶点度数）
        for (auto &pair : vertex_adjacent_edges[vid]) {
            edge_t &edge = *pair.second;
            vid_t neighbor = edge.first == vid ? edge.second : edge.first;
            // 把顶点的邻居加入边界集合
            is_boundaries[index].set_bit_unsync(neighbor);
            // 并发移除边，更新局部度数
            if (occupy_edge(pair.first)) {
                // 表示占有边
                assign_edge(index, edge.first, edge.second);
                // edge.remove();
                // 度数
                degree_updated[vid][index]++;
                degree_updated[neighbor][index]++;
                vertices_need_updated.insert(neighbor);
                vertices_need_updated.insert(vid);
            }
        }
        // 这里不需要同步吗？
        // 分配二阶边
        // 先判断顶点是否已经分配到多个分区，如果是，则判断
        for (auto &pair : vertex_adjacent_edges[vid]) {
            edge_t &edge = *pair.second;
            vid_t neighbor = edge.first == vid ? edge.second : edge.first;
            // 这里只需要关注边界集的边
            for (auto  &neighbor_pair : vertex_adjacent_edges[neighbor]) {
                vid_t e_id = neighbor_pair.first;
                edge_t &neighbor_edge = *neighbor_pair.second;
                vid_t  two_hop_neighbor = edge.first == neighbor ? edge.second : edge.first;
                // 核心集，忽略
                if (is_cores[index].get(two_hop_neighbor)) continue;
                // 不在边界集，忽略
                if (!is_boundaries[index].get(two_hop_neighbor)) continue;
                // 同样使用抢占的方式
                if (occupy_edge(e_id)) {
                    assign_edge(index, neighbor_edge.first, neighbor_edge.second);
                    // neighbor_edge.remove();
                    degree_updated[neighbor][index]++;
                    degree_updated[two_hop_neighbor][index]++;
                    vertices_need_updated.insert(neighbor);
                    vertices_need_updated.insert(two_hop_neighbor);
                }
            }
        }
        b1.Wait();

        for (auto v : vertices_need_updated) {
            // 抢占锁去更新
            if(acquire_vertex(v)) {
                size_t sum = 0;
                for (int num : degree_updated[v]) {
                    sum += num;
                }
                CHECK_GT(sum, 0);
                CHECK_GE(degrees[v], sum) << "Degree v: " << degrees[v] << ", sum:" << sum;
                // LOG(INFO) << degrees[v] - sum;
                degrees[v] = degrees[v] - sum;
                CHECK_GE(degrees[v], 0);
                // 全部置0
                fill(degree_updated[v].begin(), degree_updated[v].end(), 0);
            }
        }
        // 二次屏障，当所有顶点的度数更新完毕，每个线程再更新本地的优先队列
        b2.Wait();
        ...
        // 去除核心顶点
        vertices_need_updated.erase(vid);

        for (auto v : vertices_need_updated) {
            if (min_heaps[index].contains(v)) {
                min_heaps[index].remove(v);
            }
            if(degrees[v] == 0) continue;
            min_heaps[index].insert(degrees[v],v );
        }
        vertices_need_updated.clear();
    }
    LOG(INFO) << index << " done";
    b1.NotifyExit();
    b2.NotifyExit();
...
bool DnePartitioner::sub_get_free_vertex(vid_t &vid, vid_t index) {
...
bool DnePartitioner::get_free_vertex(vid_t &vid, size_t index) {
    //随机选择一个节点
    vid = dis(gen);
    ...
    while (count < num_vertices &&
           (degrees[vid] == 0 || is_cores[index].get(vid))) {
        vid = (vid + ++count) % num_vertices;
    }
    // LOG(INFO) << "vid: " << vid << ", degree: " << degrees[vid];
    if (count == num_vertices)
        return false;
    return true;
}

void DnePartitioner::sub_add_boundary(vid_t vid, size_t index) {
...
void DnePartitioner::add_boundary(vid_t vid) {
    // 获取到当前分区的核心集和边界集
...
void DnePartitioner::assign_edge(size_t index, vid_t from, vid_t to) {
...
size_t DnePartitioner::check_edge(const edge_t *e) {
...
void DnePartitioner::build_vertex_adjacent_edges() {
    // 遍历edges
...
//void DnePartitioner::calculate_replication_factor() {
