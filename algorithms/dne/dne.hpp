...
class DnePartitioner : public EdgePartitioner {
private:
    Barrier b1;
    Barrier b2;
    size_t cores;
    double capacity_ratio;
    // vector<priority_queue<>> degree_heaps;
//    vector<set<Vertex_element>> degree_heaps;
    double epsilon = 1;
    // new_vid, old_vid
...
//    vector<size_t> reverse_indices;
    ...
    vector<size_t> edge_lock;

    vector<vector<size_t>> degree_updated;
...
//    uint64_t max_load = 0;
    ...
    bool get_free_vertex(vid_t &vid, size_t index);
...
public:
    DnePartitioner(BaseGraph& baseGraph, const string& input, const string& algorithm,
    ...
    // 广度遍历，重新索引，用于将顶点分块
    // void re_index();
    bool occupy_edge(size_t e_id) {
        return __sync_bool_compare_and_swap(&edge_lock[e_id], 0, 1);
    }
