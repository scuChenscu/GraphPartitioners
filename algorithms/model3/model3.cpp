...
//固定随机数
// 构造函数
Model3Partitioner::Model3Partitioner(std::string input, std::string algorithm, int num_partition)
        : input(input), gen(985) {
    p = num_partition;
...
//最后一个子图就是剩下边组合而成
void Model3Partitioner::assign_remaining() {
...
void Model3Partitioner::assign_master() {
...
size_t Model3Partitioner::count_mirrors() {
    size_t result = 0;
    rep(i, p) result += is_boundarys[i].popcount();
    return result;
}

void Model3Partitioner::split() {
    // 初始化最小堆，用于存储S\C的顶点信息
