...
class OffstreamNWGPartitioner : public EdgePartitioner {
private:
    ...
    vector<vid_t> edges_buffer;

    // 在递归过程操作过的顶点，每次用完来清理
    unordered_set<vid_t> remove_vertices;

    int buffer_size;

    unordered_map<vid_t, vector<vid_t>> vertex_indices;
    ...
    void greedy_assign(vid_t index);

    void standard_greedy(edge_t &edge);

public:
    OffstreamNWGPartitioner(BaseGraph& baseGraph, const string& input, const string& algorithm,
