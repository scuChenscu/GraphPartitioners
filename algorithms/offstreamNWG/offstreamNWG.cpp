#include "offstreamNWG.hpp"

using namespace std;

//固定随机数
// 构造函数
OffstreamNWGPartitioner::OffstreamNWGPartitioner(BaseGraph& baseGraph, const string &input, const string &algorithm,
...
//最后一个子图就是剩下边组合而成
void OffstreamNWGPartitioner::assign_remaining() {
    ...
    repv(u, num_vertices) for (auto &i: adj_out[u])
            if (edges[i.v].valid()) {
                ...
                vp_set[u].insert(num_partitions - 1);
                vp_set[edges[i.v].second].insert(num_partitions - 1);
                vertex_partitions[u][num_partitions - 1] = 1;
                vertex_partitions[edges[i.v].second][num_partitions - 1] = 1;
...
void OffstreamNWGPartitioner::assign_master() {
...
size_t OffstreamNWGPartitioner::count_mirrors() {
    size_t result = 0;
    rep(i, num_partitions) result += is_boundaries[i].popcount();
    return result;
}

void OffstreamNWGPartitioner::split() {
    total_time.start();
    // 初始化最小堆，用于存储S\C的顶点信息
    ...
    stringstream ss;
    ss << "OffstreamNWG" << endl
       << "BALANCE RATIO:" << BALANCE_RATIO
       << endl;
    LOG(INFO) << ss.str();
    appendToFile(ss.str());
    LOG(INFO) << "NE allocate edges count: " << off_part.size();
    ...
    LOG(INFO) << "Allocate edges count before assign_remaining: " << assigned_edges;
    // 把剩余的边放入最后一个分区
    assign_remaining();
    ...
    // 使用贪心来划分
    LOG(INFO) << "Start window-base-greedy partitioning" << endl;
    for (int idx = 0; idx < stream_part.size(); idx++) {
        greedy_assign(idx);
        if (!remove_vertices.empty()) {
            for (vid_t vid : remove_vertices) {
                vertex_indices.erase(vid);
            }
        }
        remove_vertices.clear();
    }
    // TODO 为什么会有剩余
    LOG(INFO) << vertex_indices.size();
    for (auto &pair : vertex_indices) {
        // LOG(INFO) << pair.second.size();
        for (int i = 0; i < pair.second.size(); i++) {
            standard_greedy(off_part[pair.second[i]]);
        }
    }
    // 必然为空
...
void OffstreamNWGPartitioner::greedy_assign(vid_t idx) {
    auto &edge = stream_part[idx];
    ...
    if (!intersections.empty()) {
        partition = leastLoad(intersections); // TODO 这一步不会引入新的复制因子，只需要考虑负载问题
        ...
        fp.insert(partition);
        sp.insert(partition);
    } else if (intersections.empty() && !unions.empty()) { // TODO 这一步必然会引入一个新的复制因子
        ...
        fp.insert(partition);
        sp.insert(partition);
    } else if (fp.empty() && !sp.empty()) { // TODO 这一步会引入一个复制因子
        ...
        fp.insert(partition);
        sp.insert(partition);
        if (remove_vertices.contains(first)) {
            return;
        }
        remove_vertices.insert(first);
        // 找到是否有跟fp关联未分配的边，执行分配
        if (vertex_indices.contains(first)) {
            vector<vid_t> index_set = vertex_indices[first];
            for (vid_t i : index_set) {
                if (stream_part[i].valid()) {
                    greedy_assign(i);
                }
            }
        }
        ...
        fp.insert(partition);
        sp.insert(partition);
        if (remove_vertices.contains(second)) {
            return;
        }
        remove_vertices.insert(second);
        if (vertex_indices.contains(second)) {
            vector<vid_t> index_set = vertex_indices[second];
            for (vid_t i : index_set) {
                if (stream_part[i].valid()) {
                    greedy_assign(i);
                }
            }
            vertex_indices.erase(first); // TODO 这里可能是一个递归过程
        }
    } else if (fp.empty() && sp.empty()){ // TODO 这一步会引入两个复制因子，所以暂时不分配，加入到缓存区
        // 找出occupied最小值所在的下标
//            int min = occupied[0];
...
//            }
        ...
        // 把边加入两个顶点中
        if (!vertex_indices.contains(first)) {
            vertex_indices[first] = {};
        }
        vertex_indices[first].push_back(idx);
        if (!vertex_indices.contains(second)) {
            vertex_indices[second] = {};
        }
        vertex_indices[second].push_back(idx);
    }
}

void OffstreamNWGPartitioner::reindex() {
    // 随机选择顶点，进行广度遍历，重新索引
    vid_t index = 0;
    vid_t vid = dis(gen);
...
bool OffstreamNWGPartitioner::get_target_vertex(vid_t &vid) {
...
bool OffstreamNWGPartitioner::get_free_vertex(vid_t &vid) {
    //随机选择一个节点
    vid = dis(gen);
...
void OffstreamNWGPartitioner::occupy_vertex(vid_t vid, vid_t d) {
...
size_t OffstreamNWGPartitioner::check_edge(const edge_t *e) {
...
void OffstreamNWGPartitioner::assign_edge(size_t partition, vid_t from, vid_t to) {
    // TODO 记录哪条边被分配了
...
void OffstreamNWGPartitioner::add_boundary(vid_t vid) {
    // 获取到当前分区的核心集和边界集
            ...
            if (edges[neighbors[i].v].valid()) {
                ...
                if (is_core.get(u)) { // 如果顶点在核心集中
                    assign_edge(current_partition, direction ? vid : u,
                                direction ? u : vid);
                    vp_set[u].insert(current_partition);
                    vp_set[vid].insert(current_partition);
                    ...
                    neighbors.pop_back();
                } else if (is_boundary.get(u) && occupied[current_partition] < capacity) { // 如果顶点在边界集中，并且当前分区负载没有达到上限
                    // 将边加入边集
...
size_t OffstreamNWGPartitioner::leastLoad(set<size_t> partition_set) {
...
void OffstreamNWGPartitioner::standard_greedy(edge_t &edge) {
    if (!edge.valid()) return;
    vid_t first = edge.first;
    vid_t second = edge.second;
    // TODO 这个移动到NE开始维护
