...
//固定随机数
// 构造函数
Model1Partitioner::Model1Partitioner(std::string input, std::string algorithm, int num_partition)
        : input(input), gen(985) {
    p = num_partition;
...
//最后一个子图就是剩下边组合而成
void Model1Partitioner::assign_remaining() {
...
void Model1Partitioner::assign_master() {
...
size_t Model1Partitioner::count_mirrors() {
    size_t result = 0;
    rep(i, p) result += is_boundarys[i].popcount();
    return result;
}

void Model1Partitioner::split() {
    // 初始化最小堆，用于存储S\C的顶点信息
